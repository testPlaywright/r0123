...
Cypress.Commands.add("clickButton",function(btnName) {
  let flag = false
  objTiersMenu.getButton().each(($el, index)=>{
    cy.log($el.text())
    if(($el.text()) === btnName)
    cy.log("after if condition" +$el.text())
    objTiersMenu.getButton().eq(index).click()
    flag = true;
    return false;
  })
